...
// Вывести форму
pw.println("<form action='/MessengerServlet/' method='post'>Inpute name:<input type='text' name='name' value=''><input type='submit' value='In chat'>");
...
errorMessage = "Имя пользователя не может быть пустым!";
	...
	if (activeUsers.size()>=N) {
		// Пустое имя недопустимо - сообщить об ошибке
		errorMessage = "wrong number user";
		}else{
...
response.sendRedirect(response.encodeRedirectURL("/MessengerServlet/"));
}
}
...
response.sendRedirect(response.encodeRedirectURL("/MessengerServlet/view.htm"));
